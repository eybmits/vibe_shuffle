...
\newcommand{\scalebar}[4]{% leftcolor, rightcolor, leftlabel, rightlabel
  \begin{tikzpicture}
    ...
    \node[font=\small\bfseries, text=ink] at (1.7,0.4) {#3};
    \node[font=\small\bfseries, text=ink] at (9.2,0.4) {#4};
    \node[font=\large, text=soft] at (5.5,0.4) {$\longleftrightarrow$};
  \end{tikzpicture}}
...
\begin{tikzpicture}[node distance=8mm]
  ...
  \node[box, below=of b2] (b3)
    {\head{While a song plays, we compare to that resting rhythm}
     ...
     regular? This sets a \emph{base} energy level.};
  ...
  \node[result, below=8mm of bar] (b5)
    {\head{Result: an ``energy'' level}
     The heartbeat sets the base, and moving along always pushes the energy
     higher.};
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \node[box, below=42mm of b1] (pick)
    {\head{Pick one song (skipping ones just played)}
     It plays for 60 seconds, then you rate it. This repeats, alternating
     Vibe and Random blocks.};

  \node[box, below=of pick, fill=white, text=soft, font=\footnotesize] (note)
    {The whole point of the study: do the mood-matched ``Vibe'' songs feel
     better than the random ones?};
  ...
  \draw[arrow] (a1) -- (a2);
  \draw[arrow] (a2) -- (c1.north);
  \draw[arrow] (a2) -- (c2.north);
  \draw[arrow] (a2) -- (c3.north);
  \draw[arrow] (a2) -- (c4.north);
  \draw[arrow] (c1.south) -- (a3);
  \draw[arrow] (c2.south) -- (a3);
  \draw[arrow] (c3.south) -- (a3);
  \draw[arrow] (c4.south) -- (a3);
  \draw[arrow] (a3) -- (b1);
  \draw[arrow] (b1) -- (vibe);
  \draw[arrow] (b1) -- (rand);
  \draw[arrow] (vibe) -- (pick);
  \draw[arrow] (rand) -- (pick);
  \draw[arrow] (pick) -- (note);
\end{tikzpicture}
\end{center}
