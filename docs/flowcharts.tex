...
\newcommand{\scalebar}[4]{% leftcolor, rightcolor, leftlabel, rightlabel
  \begin{tikzpicture}
    ...
    \node[font=\small\bfseries, text=ink, anchor=mid] at (1.7,0.4) {#3};
    \node[font=\small\bfseries, text=ink, anchor=mid] at (9.2,0.4) {#4};
    \node[font=\large, text=soft, anchor=mid] at (5.5,0.4) {$\longleftrightarrow$};
  \end{tikzpicture}}
...
\begin{tikzpicture}[node distance=7mm]
  ...
  \node[result, below=of a3] (b1)
    {\head{Your average mood during the song lands in one corner}
     ...
     Together they are averaged over the 60 s listening window.};

  \node[mode, fill=song!10, draw=song, text width=6.1cm,
        below=11mm of b1, xshift=-3.6cm] (vibe)
    {\textbf{Vibe mode}\\[2pt]
     Pick mostly from songs in the corner of \emph{your} average 60 s state ---
     music that matches how the last song affected you.};
