...
A growing body of work in affective computing estimates affect from facial
expression and from autonomic signals such as heart-rate variability, and
organises it along the two dimensions of valence and arousal---a low-dimensional
\emph{core affect} rather than discrete, labelled emotions~\cite{russell1980,
posner2005}. Bringing such estimates to bear on music
...
application ever receiving the audio stream. Throughout, the pipeline estimates a
continuous valence--arousal coordinate---a behavioural-physiological proxy for
\emph{core affect}~\cite{russell1980}---rather than classifying discrete, named
emotions.

\subsection{Valence from facial expression}
A MediaPipe Face Landmarker~\cite{mediapipe2019} produces per-frame blendshape
activations that are reduced to three facial-\emph{expression} scores we label
\emph{happy}, \emph{tense}, and \emph{sad}---measures of facial display, not of
inferred feeling. A slowly learned personal neutral baseline is subtracted to correct for
...
estimate. The export also retains a rhythm diagnostic for exploratory analysis.
It is not used as the arousal driver because it measures regularity rather than
baseline-relative autonomic activation. The sensor is strictly optional; the
...
\texttt{physiology\_arousal} & baseline-relative HR/RMSSD arousal estimate \\
\texttt{physiology\_coherence} & exported cardiac-coherence diagnostic, not a selection driver \\
\texttt{physiology\_quality} & heart-rate signal-quality flag \\
\texttt{rating\_fit\_1\_to\_7} & primary outcome: mood-fit \\
\texttt{rating\_like\_1\_to\_7} & control: liking \\
\bottomrule
\end{tabularx}
\end{table}
...
Several limitations bound the claims. The system does not recognise discrete
emotions: it estimates a low-dimensional core-affect proxy from a
facial-expression display, autonomic arousal, and head motion. Facial movements
are not a reliable read-out of felt emotion---the same expression recurs across
states and the same state across expressions~\cite{barrett2019}---and
physiological arousal is undifferentiated and valence-blind, raised alike by
excitement, exertion, or the played music itself; the detected state is therefore
a behavioural-physiological correlate of affect rather than affect itself.
Because both axes are referenced to a per-session personal baseline, the system
tracks change relative to the listener's starting point rather than an absolute
state: a listener who begins aroused has that state absorbed into the neutral
baseline, so the absolute initial affect is unobserved---a self-reported baseline
(e.g.\ a Self-Assessment Manikin) would anchor it. The estimator is a transparent
heuristic rather than a validated clinical affect measure; facial valence is
sensitive to lighting and occlusion, and the arousal channel depends on a
compatible sensor that not every participant will own. The pool is small and fixed by design, so
